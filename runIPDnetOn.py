...
import IPDnet.lightweight_model as at_model
...
class MyModel(LightningModule):
    ...
    #End-fire weighted loss based on ITD
    def cal_endfire_weighted_loss(self, pred_batch=None, gt_batch=None,batch_idx=None):
        gt_weight = 1.5 - torch.sin(gt_batch[0])  # [nb,nt,nea,nsrc]: 모든 batch, time frame에 대한 DoA에 따른 weight
        nb,nt,nea,nsrc = gt_weight.shape
        reshape_gt_weight = gt_weight.reshape(nb*nt, nea, nsrc)  # [nb*nt,nea,nsrc]
        reshape_gt_weight = reshape_gt_weight[:,1,:]  # [nb*nt,2] azimuth만 가져옴
        reshape_gt_weight = reshape_gt_weight.unsqueeze(-1).repeat(1,1,512)  # gt_weight shape 맞춰주기
        ...
        ipd_gt_batch = ipd_gt_batch.reshape(nb*nt,-1,nsrc).permute(0,2,1)
        pred_batch = pred_batch.to(self.dev)
        best_metric, best_perm = permutation_invariant_training(pred_batch, ipd_gt_batch, self.MSE_loss, eval_func='min')  # PIT 적용
        pred_batch= pit_permutate(pred_batch, best_perm)
        
        weighted_mse_loss = torch.mean(((pred_batch - ipd_gt_batch)**2) * reshape_gt_weight)
        weighted_mse_loss
        
        return weighted_mse_loss   
    
    
    #End-fire weighted loss based on IPD (+ frequency weighting)
    def cal_freq_endfire_weighted_loss(self, pred_batch=None, gt_batch=None,batch_idx=None):
        ipd_gt_batch = gt_batch[1]  # gt_batch[1] -> gt_ipd
        nb, nt, _, nmic, nsrc = pred_batch.shape
        pred_batch = pred_batch.reshape(nb*nt, -1, nsrc).permute(0,2,1)
        ipd_gt_batch = ipd_gt_batch.reshape(nb*nt,-1,nsrc).permute(0,2,1)  # [92,2,512]
        pred_batch = pred_batch.to(self.dev)
        best_metric, best_perm = permutation_invariant_training(pred_batch, ipd_gt_batch, self.MSE_loss, eval_func='min')
        pred_batch= pit_permutate(pred_batch, best_perm)  # [92,2,512]
        
        ## DoA에 따라 weight 부여
        doa_weight = torch.sin(gt_batch[0])  # gt_batch[0] -> gt_doa [4,23,2,2]
        nb,nt,nea,nsrc = doa_weight.shape  # [4,23,2,2]
        reshape_doa_weight = doa_weight.reshape(nb*nt, nea, nsrc)  # [92,2,2]
        reshape_doa_weight = reshape_doa_weight[:,1,:]  # [92,2]
        reshape_doa_weight = reshape_doa_weight.unsqueeze(-1).repeat(1,1,512)  # [92,2,512]
        
        ## Frequency (bin index)에 따라 weight 부여
        freq_weight = torch.arange(1,257,1)  # 1부터 256까지, self.fre_range_used = range(1, int(self.nfft/2)+1, 1)
        freq_weight = torch.cat([freq_weight, freq_weight])  # [512]
        freq_weight = freq_weight.unsqueeze(0).unsqueeze(0).to(self.dev)  # [1,1,512]
        
        ## DoA 및 Frequency 고려하여 weight 부여
        freq_doa_weight = reshape_doa_weight *  freq_weight  # [92,2,512]
        final_weight = 512 / (freq_doa_weight + 256) - 0.5  # [92,2,512]
        # final_weight = 256 / (freq_doa_weight + 256) + 0.5  # [92,2,512]
        # weighted_mse_loss = torch.mean((final_weight * (pred_batch - ipd_gt_batch))**2)
        weighted_mse_loss = torch.mean(final_weight * ((pred_batch - ipd_gt_batch)**2))
